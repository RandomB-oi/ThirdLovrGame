...
module.__parent = "Framework/Classes/Instances/Entity"
...
function module:Emit(amount)
    ...
    for i = 1, (amount or 1) do
        local newParticle = {
            Position = self.Position + Vector2.new(math.random(-sizeRange.X, sizeRange.X), math.random(-sizeRange.Y, sizeRange.Y)),
...
function module:Draw()
    for i, particle in ipairs(self.Particles) do
        ...
        lovr.graphics.rectangle("fill", particle.Position.X-size/2, particle.Position.Y-size/2, size, size)
    end
end
