...
float getFogAlpha(vec3 worldPoint) {
	float dist = distance(worldPoint, CameraPositionWorld);
	// if (dist >= 1000) return -1; // skybox
...
vec4 lovrmain() {
	...
	vec4 appliedSunlight = max(dot(norm, sunDirection), 0.0) * sunLightColor;

	float fogAlpha = getFogAlpha(PositionWorld);
	vec4 textureColor = getPixel(ColorTexture, UV);
	vec4 baseColor = (Color * textureColor) * (ambience + appliedSunlight);

    // float distFromLight = distance(PositionWorld, lightPosition);
    // if (distFromLight <= lightRange) {
    //     baseColor += (lightColor * lightBrightness * (1-(distFromLight/lightRange)));
    // }

	return lerpColor(baseColor, fogColor, fogAlpha);
};
